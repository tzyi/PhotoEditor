...
| `-t`, `--threads` | 否 | 並行執行緒數（預設為 CPU 核心數，最多 8，可自行調整） |
...
3. 使用多執行緒加速（例如 16 執行緒）
```bash
python compress_images.py -i "G:\0002_相簿\日期相簿_26\260203" -o "C:\Users\arif1\Downloads\未命名的轉存\compress" -q 40 -t 16
```
...
- 支援多執行緒並行壓縮，大量圖片時可大幅提升速度，建議依電腦效能調整 `-t` 參數。
